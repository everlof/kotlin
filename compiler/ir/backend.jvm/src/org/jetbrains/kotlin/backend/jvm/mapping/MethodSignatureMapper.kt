...
class MethodSignatureMapper(private val context: JvmBackendContext) {
    ...
    private fun mapSignature(function: IrFunction, skipGenericSignature: Boolean, skipSpecial: Boolean = false): JvmMethodGenericSignature {
        ...
        for (i in function.contextReceiverParametersCount until function.valueParameters.size) {
            ...
            writeParameter(sw, parameter.isSkippedInGenericSignature, type, function)
        }
    ...
    private fun tryDeserializeSignature(descriptor: DeserializedCallableMemberDescriptor): JvmMethodGenericSignature? {
        val containerSource = descriptor.containerSource
        if (containerSource !is JvmPackagePartSource && containerSource !is KotlinJvmBinarySourceElement) return null
